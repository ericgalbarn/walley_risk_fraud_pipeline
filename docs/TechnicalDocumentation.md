...
**Insight:** Transactions with `distance_from_home_km > 100km` combined with an off-hours flag showed an 87% higher average risk score, consistent with unauthorized remote access patterns.
...
**Insight:** Isolated 629 velocity-flagged transactions. 78% of these bursts occurred between 1:00–4:00 AM, consistent with attackers exploiting victims' sleeping hours before detection.
...
**Insight:** Flagged 40 transactions across 8 distinct mule rings. All 8 mule beneficiary accounts were less than 15 days old, supporting the hypothesis that rings rely on newly opened, disposable accounts.
...
**Insight:** 1,422 transactions matched this pattern — 67.7% of all flagged anomalies, making structuring just under the biometric threshold the single largest fraud vector in the dataset.
...
From **18,164** cleaned transactions, the RegTech layer flagged **~2,100 suspicious transactions** (**~11.5%** flag rate).

| Risk Rule | Triggered Count | Fraud Pattern & Business Context |
|---|---|---|
| `biometric_evasion_rule` | 1,422 | Transfers of 9M–9.99M VND to newly created beneficiaries, avoiding mandatory face-matching under Decision 2345. |
| `velocity_rule` | 629 | Rapid 5-minute-window bursts, typical of stolen session tokens or ATO cash-out attempts. |
| `circular_17_mule_network_rule` | 40 | 8 distinct mule rings collecting funds from ≥4 victims within 1 hour. |
| `biometric_structuring_rule` | 28 | Cross-border FATF-flagged transactions layered with biometric evasion. |

> **Reconciliation note:** Rule counts (1,422 + 629 + 40 + 28 = 2,119) exceed the total flagged transaction count (~2,100) because some transactions trigger more than one rule simultaneously. `is_flagged` counts unique transactions; the rule breakdown counts individual rule *triggers*.
...
**Finding:** 1,422 transactions bypassed biometric checks by staying just under the 10M VND threshold.
...
**Finding:** 40 transactions tied to high-speed mule aggregation networks.
